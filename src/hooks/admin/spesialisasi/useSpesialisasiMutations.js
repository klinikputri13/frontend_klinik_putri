...
const useSpesialisasiMutations = () => {
  ...
  // Menambahkan fungsi activate
  const activate = useMutation({
    mutationFn: (id) => SpesialisasiRepository.activate(id),
    onSuccess: (data) => {
      toast.success(`Spesialisasi ${data.nama} berhasil diaktifkan!`, {
        duration: 4000,
        position: 'top-center',
        style: {
          borderRadius: '10px',
  ...
  return { create, edit, remove, activate };
}
