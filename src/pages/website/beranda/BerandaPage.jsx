...
import Ilustration from "@/assets/images/images.png";
    ...
    <section className="px-4 py-10 md:px-32">
      ...
      <section className="grid grid-cols-1 gap-4 px-6 capitalize rounded-lg lg:grid-cols-3 bg-primary md:px-14 py-7">
        <div className="flex flex-col justify-center w-full col-span-2 text-white space-y-7">
          ...
          <a href="#" className="px-6 py-2 font-bold bg-white rounded-lg text-primary w-fit">lihat spesialisasi</a>
        </div>
        <div className="flex justify-center">
          {/* Perbaiki tampilan gambar agar tidak stretch */}
          <img src={Ilustration} alt="images" className="object-contain max-w-md" />
        </div>
      </section>
      {/* Spesialisasi Cards - responsive grid */}
      <section className="grid grid-cols-1 gap-6 py-10 sm:grid-cols-2 lg:grid-cols-3">
        {spesialisasiTakeAllQueries?.data?.map((spesialisasi) => (
          <article className="capitalize bg-white rounded-lg shadow-xl">
            ...
            <div className="flex flex-col gap-5 p-5">
              ...
              <a href={`/user/reservasi/${spesialisasi.id}`} className="self-end px-6 py-2 font-medium text-left text-white rounded-lg bg-primary w-fit">reservasi</a>
            </div>
          </article>
        ))}
      </section>
